  ...
  return (
    <div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <ChartCard title="Created by Month" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) FROM Task GROUP BY 1 ORDER BY 1;`} currentUser={currentUser} entity="Tasks" tableData={d.createdByMonth}><ResponsiveContainer width="100%" height={180}><LineChart data={d.createdByMonth} margin={{left:-20}}><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Line type="monotone" dataKey="count" stroke="#f59e0b" strokeWidth={2} dot={false}/></LineChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Weekly Task Volume" description="Last 8 weeks" sql={`SELECT DATE_TRUNC('week',created_date) w, COUNT(*) FROM Task GROUP BY 1 ORDER BY 1 DESC LIMIT 8;`} currentUser={currentUser} entity="Tasks" tableData={d.weekly}><ResponsiveContainer width="100%" height={180}><BarChart data={d.weekly} margin={{left:-20}}><XAxis dataKey="week" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#6366f1" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
        <ChartCard title="Open Task Backlog" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) FROM Task WHERE status='open' GROUP BY 1 ORDER BY 1;`} currentUser={currentUser} entity="Tasks" tableData={d.backlog}><ResponsiveContainer width="100%" height={180}><AreaChart data={d.backlog} margin={{left:-20}}><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Area type="monotone" dataKey="open" stroke="#ef4444" fill="#fee2e2"/></AreaChart></ResponsiveContainer></ChartCard>
