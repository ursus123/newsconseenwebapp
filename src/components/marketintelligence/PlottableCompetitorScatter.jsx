 ...
 *
 * Renders a scatter plot of competitors: X = distance (km), Y = rating (0–5).
 * Uses Plottable.js for zoom + pan — useful when there are 10+ competitors.
 *
 * Props:
 *   competitors  Array<{ name, distance_km, rating?, address? }>
 *   radiusKm     number
 */
...
export default function PlottableCompetitorScatter({ competitors, radiusKm }) {
  ...
  useEffect(() => {
    ...
    colorScale.range(["#10b981", "#f59e0b", "#ef4444"]);

    xScale.domainMin(0);
    xScale.domainMax(radiusKm + 2);
    yScale.domainMin(0);
    ...
    const tooltip = new Plottable.Components.Label("Drag to pan · Scroll to zoom", "0");
    tooltip.xAlignment("right");

    // Pointer for tooltip on hover
    ...
    pointer.onPointerMove(point => {
      const nearestEntity = scatter.entityNearest(point);
      if (nearestEntity) {
        const d = nearestEntity.datum;
    ...
    // Pan + zoom on X axis only (distance axis)
