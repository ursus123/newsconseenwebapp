    ...
    <div className="mt-8 border border-slate-200 rounded-2xl overflow-hidden">
      ...
      {open && (
        <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <ChartCard title="By Tier" sql={`SELECT enterprise_tier, COUNT(*) FROM Enterprise GROUP BY enterprise_tier ORDER BY 2 DESC;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><BarChart data={d.byTier} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#8b5cf6" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="By Status" sql={`SELECT status, COUNT(*) FROM Enterprise GROUP BY status;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byStatus} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="count" nameKey="name" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>{d.byStatus.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Operating Status" sql={`SELECT operating_status, COUNT(*) FROM Enterprise GROUP BY operating_status;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byOpStatus} cx="50%" cy="50%" innerRadius={40} outerRadius={70} dataKey="count" nameKey="name" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>{d.byOpStatus.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="By Country" sql={`SELECT country, COUNT(*) FROM Enterprise GROUP BY country ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><BarChart data={d.byCountry} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#10b981" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="By Region" sql={`SELECT region, COUNT(*) FROM Enterprise GROUP BY region ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><BarChart data={d.byRegion} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#06b6d4" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="By City (Top 10)" sql={`SELECT city, COUNT(*) FROM Enterprise GROUP BY city ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Enterprises">{d.byCity.length>0?<ResponsiveContainer width="100%" height={180}><BarChart data={d.byCity} layout="vertical" margin={{left:0,right:10}}><XAxis type="number" tick={{fontSize:10}} allowDecimals={false}/><YAxis dataKey="name" type="category" tick={{fontSize:10}} width={90}/><Tooltip/><Bar dataKey="count" fill="#f59e0b" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer>:<p className="text-xs text-slate-400 text-center py-16">No city data</p>}</ChartCard>
          <ChartCard title="Enterprise Subtype" sql={`SELECT enterprise_subtype, COUNT(*) FROM Enterprise GROUP BY enterprise_subtype ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Enterprises">{d.bySubtype.length>0?<ResponsiveContainer width="100%" height={180}><BarChart data={d.bySubtype} layout="vertical" margin={{left:0,right:10}}><XAxis type="number" tick={{fontSize:10}} allowDecimals={false}/><YAxis dataKey="name" type="category" tick={{fontSize:10}} width={100}/><Tooltip/><Bar dataKey="count" fill="#6366f1" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer>:<p className="text-xs text-slate-400 text-center py-16">No subtype data</p>}</ChartCard>
          <ChartCard title="SIC Division" sql={`SELECT sic_division, COUNT(*) FROM Enterprise GROUP BY sic_division ORDER BY 2 DESC;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><BarChart data={d.bySicDiv} layout="vertical" margin={{left:0,right:10}}><XAxis type="number" tick={{fontSize:10}} allowDecimals={false}/><YAxis dataKey="name" type="category" tick={{fontSize:10}} width={100}/><Tooltip/><Bar dataKey="count" fill="#ec4899" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="New Enterprises by Month" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) FROM Enterprise GROUP BY 1 ORDER BY 1;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><LineChart data={d.newByMonth} margin={{left:-20}}><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Line type="monotone" dataKey="count" stroke="#3b82f6" strokeWidth={2} dot={false}/></LineChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Cumulative Enterprise Count" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) OVER(ORDER BY DATE_TRUNC('month',created_date)) total FROM Enterprise WHERE created_date IS NOT NULL;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><AreaChart data={d.cumul} margin={{left:-20}}><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Area type="monotone" dataKey="total" stroke="#8b5cf6" fill="#ede9fe"/></AreaChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Website Coverage" sql={`SELECT CASE WHEN website IS NOT NULL AND website!='' THEN 'Has Website' ELSE 'No Website' END, COUNT(*) FROM Enterprise GROUP BY 1;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={[{name:"Has Website",count:d.withSite},{name:"No Website",count:d.withoutSite}]} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="count" nameKey="name" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>{[0,1].map(i=><Cell key={i} fill={PALETTE[i]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="HQ vs Branches" sql={`SELECT enterprise_tier, COUNT(*) FROM Enterprise GROUP BY enterprise_tier;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={[{name:"HQ",count:d.hqCount},{name:"Branch",count:d.branchCount},{name:"Other",count:enterprises.length-d.hqCount-d.branchCount}]} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{[0,1,2].map(i=><Cell key={i} fill={PALETTE[i]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Type Pie" sql={`SELECT enterprise_type, COUNT(*) FROM Enterprise GROUP BY enterprise_type;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byType} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byType.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Tier Pie" sql={`SELECT enterprise_tier, COUNT(*) FROM Enterprise GROUP BY enterprise_tier;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byTier} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byTier.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Country Pie" sql={`SELECT country, COUNT(*) FROM Enterprise GROUP BY country ORDER BY 2 DESC LIMIT 8;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byCountry.slice(0,8)} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byCountry.slice(0,8).map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Subtype Pie" sql={`SELECT enterprise_subtype, COUNT(*) FROM Enterprise GROUP BY enterprise_subtype ORDER BY 2 DESC LIMIT 8;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.bySubtype.slice(0,8)} cx="50%" cy="50%" innerRadius={30} outerRadius={70} dataKey="count" nameKey="name">{d.bySubtype.slice(0,8).map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          ...
          <ChartCard title="Region Pie" sql={`SELECT region, COUNT(*) FROM Enterprise GROUP BY region ORDER BY 2 DESC LIMIT 8;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byRegion.slice(0,8)} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byRegion.slice(0,8).map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Operating Status Trend" sql={`SELECT operating_status, COUNT(*) FROM Enterprise WHERE operating_status IS NOT NULL GROUP BY operating_status ORDER BY 2 DESC;`} currentUser={currentUser} entity="Enterprises"><ResponsiveContainer width="100%" height={180}><BarChart data={d.byOpStatus} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#f97316" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
        </div>
      )}
    </div>
  );
