    ...
    <div className="mt-8 border border-slate-200 rounded-2xl overflow-hidden">
      ...
      {open && (
        <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <ChartCard title="By Enterprise Type" sql={`SELECT enterprise_type, COUNT(*) FROM Enterprise GROUP BY enterprise_type ORDER BY 2 DESC;`} currentUser={currentUser} entity="Enterprises" tableData={d.byType}><ResponsiveContainer width="100%" height={180}><BarChart data={d.byType} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" radius={[4,4,0,0]}>{d.byType.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Bar></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="By Tier" sql={`SELECT enterprise_tier, COUNT(*) FROM Enterprise GROUP BY enterprise_tier ORDER BY 2 DESC;`} currentUser={currentUser} entity="Enterprises" tableData={d.byTier}><ResponsiveContainer width="100%" height={180}><BarChart data={d.byTier} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#8b5cf6" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
          ...
          <ChartCard title="By Region" sql={`SELECT region, COUNT(*) FROM Enterprise GROUP BY region ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Enterprises" tableData={d.byRegion}><ResponsiveContainer width="100%" height={180}><BarChart data={d.byRegion} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#06b6d4" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
          ...
          <ChartCard title="New Enterprises by Month" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) FROM Enterprise GROUP BY 1 ORDER BY 1;`} currentUser={currentUser} entity="Enterprises" tableData={d.newByMonth}><ResponsiveContainer width="100%" height={180}><LineChart data={d.newByMonth} margin={{left:-20}}><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Line type="monotone" dataKey="count" stroke="#3b82f6" strokeWidth={2} dot={false}/></LineChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Cumulative Enterprise Count" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) OVER(ORDER BY DATE_TRUNC('month',created_date)) total FROM Enterprise WHERE created_date IS NOT NULL;`} currentUser={currentUser} entity="Enterprises" tableData={d.cumul}><ResponsiveContainer width="100%" height={180}><AreaChart data={d.cumul} margin={{left:-20}}><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Area type="monotone" dataKey="total" stroke="#8b5cf6" fill="#ede9fe"/></AreaChart></ResponsiveContainer></ChartCard>
