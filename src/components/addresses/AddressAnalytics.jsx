...
export default function AddressAnalytics({ addresses = [], currentUser }) {
    ...
    <div className="mt-8 border border-slate-200 rounded-2xl overflow-hidden">
      ...
      {open && (
        <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <ChartCard title="GPS Coverage" description="Geocoded vs missing coordinates" sql={`SELECT CASE WHEN latitude IS NOT NULL AND longitude IS NOT NULL THEN 'Geocoded' ELSE 'Missing GPS' END, COUNT(*) FROM Address GROUP BY 1;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={[{name:"Geocoded",count:d.withGPS},{name:"Missing GPS",count:d.noGPS}]} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="count" nameKey="name" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>{[0,1].map(i=><Cell key={i} fill={["#10b981","#f59e0b"][i]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="By Country" sql={`SELECT country, COUNT(*) FROM Address GROUP BY country ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><BarChart data={d.byCountry} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#10b981" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
          ...
          <ChartCard title="By Region" sql={`SELECT region, COUNT(*) FROM Address GROUP BY region ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><BarChart data={d.byRegion} layout="vertical" margin={{left:0,right:10}}><XAxis type="number" tick={{fontSize:10}} allowDecimals={false}/><YAxis dataKey="name" type="category" tick={{fontSize:10}} width={80}/><Tooltip/><Bar dataKey="count" fill="#8b5cf6" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="New Addresses by Month" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) FROM Address GROUP BY 1 ORDER BY 1;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><LineChart data={d.newByMonth} margin={{left:-20}}><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Line type="monotone" dataKey="count" stroke="#f59e0b" strokeWidth={2} dot={false}/></LineChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Cumulative Addresses" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) OVER(ORDER BY DATE_TRUNC('month',created_date)) total FROM Address WHERE created_date IS NOT NULL;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><AreaChart data={d.cumul} margin={{left:-20}}><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Area type="monotone" dataKey="total" stroke="#06b6d4" fill="#cffafe"/></AreaChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Active vs Archived" sql={`SELECT status, COUNT(*) FROM Address GROUP BY status;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.activeVsArchived} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="count" nameKey="name" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>{[0,1].map(i=><Cell key={i} fill={["#10b981","#94a3b8"][i]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Type Pie" sql={`SELECT address_type, COUNT(*) FROM Address GROUP BY address_type ORDER BY 2 DESC;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byType} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byType.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Country Pie" sql={`SELECT country, COUNT(*) FROM Address GROUP BY country ORDER BY 2 DESC LIMIT 8;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byCountry.slice(0,8)} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byCountry.slice(0,8).map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="City Pie" sql={`SELECT city, COUNT(*) FROM Address GROUP BY city ORDER BY 2 DESC LIMIT 8;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byCity.slice(0,8)} cx="50%" cy="50%" innerRadius={30} outerRadius={70} dataKey="count" nameKey="name">{d.byCity.slice(0,8).map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Address Line 2 Coverage" sql={`SELECT CASE WHEN address_line2 IS NOT NULL AND address_line2!='' THEN 'Full Address' ELSE 'Line 1 Only' END, COUNT(*) FROM Address GROUP BY 1;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={[{name:"Full Address",count:d.withLine2},{name:"Line 1 Only",count:d.noLine2}]} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="count" nameKey="name" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>{[0,1].map(i=><Cell key={i} fill={PALETTE[i]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Supplier Addresses by City" sql={`SELECT city, COUNT(*) FROM Address WHERE address_type='supplier' GROUP BY city ORDER BY 2 DESC LIMIT 8;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><BarChart data={cnt(addresses.filter(a=>a.address_type==="supplier"),"city")} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#f97316" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Government Addresses by Region" sql={`SELECT region, COUNT(*) FROM Address WHERE address_type='government' GROUP BY region ORDER BY 2 DESC;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><BarChart data={cnt(addresses.filter(a=>a.address_type==="government"),"region")} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" fill="#6366f1" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer></ChartCard>
          ...
          <ChartCard title="Region Pie" sql={`SELECT region, COUNT(*) FROM Address GROUP BY region ORDER BY 2 DESC LIMIT 8;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byRegion.slice(0,8)} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byRegion.slice(0,8).map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="GPS Coverage Bar" sql={`SELECT CASE WHEN latitude IS NOT NULL THEN 'Geocoded' ELSE 'Missing' END, COUNT(*) FROM Address GROUP BY 1;`} currentUser={currentUser} entity="Addresses"><ResponsiveContainer width="100%" height={180}><BarChart data={[{name:"Geocoded",count:d.withGPS},{name:"Missing GPS",count:d.noGPS}]} margin={{left:-20}}><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar dataKey="count" radius={[4,4,0,0]}>{[0,1].map(i=><Cell key={i} fill={["#10b981","#f59e0b"][i]}/>)}</Bar></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Addresses with Full Data" description="Line1 + City + Country + GPS" sql={`SELECT COUNT(*) FROM Address WHERE address_line1 IS NOT NULL AND city IS NOT NULL AND country IS NOT NULL AND latitude IS NOT NULL AND longitude IS NOT NULL;`} currentUser={currentUser} entity="Addresses"><div className="flex items-center justify-center min-h-[180px]"><div className="text-center"><p className="text-4xl font-bold text-emerald-600">{addresses.filter(a=>a.address_line1&&a.city&&a.country&&a.latitude&&a.longitude).length}</p><p className="text-sm text-slate-500 mt-1">fully complete addresses</p><p className="text-xs text-slate-400 mt-1">of {addresses.length} total</p></div></div></ChartCard>
        </div>
      )}
    </div>
  );
