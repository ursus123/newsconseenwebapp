    ...
    <div className="mt-8 border border-slate-200 rounded-2xl overflow-hidden">
      ...
      {open && (
        <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <ChartCard title="Cumulative Addresses" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) OVER(ORDER BY DATE_TRUNC('month',created_date)) total FROM Address WHERE created_date IS NOT NULL;`} currentUser={currentUser} entity="Addresses" tableData={d.cumul}><ResponsiveContainer width="100%" height={180}><AreaChart data={d.cumul} margin={{left:-20}}><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Area type="monotone" dataKey="total" stroke="#06b6d4" fill="#cffafe"/></AreaChart></ResponsiveContainer></ChartCard>
