// ==============================================================
// AgentDashboard - Idjwi closed-loop agent status panel
// ==============================================================
...
const AGENT_META = {
  daily_health_check: { icon: Activity, color: "blue", label: "Daily Health Check", phase: "9", schedule: "Daily 6am" },
  import_cleanup: { icon: RefreshCw, color: "amber", label: "Import Cleanup", phase: "9", schedule: "After import" },
  relationship_repair: { icon: Network, color: "teal", label: "Relationship Repair", phase: "9", schedule: "Daily 3am" },
  risk_monitoring: { icon: AlertCircle, color: "rose", label: "Risk Monitoring", phase: "9", schedule: "Daily 7am" },
  operations: { icon: Activity, color: "blue", label: "Operations Agent", phase: "9", schedule: "Every 15 min" },
  revenue: { icon: TrendingUp, color: "emerald", label: "Revenue Agent", phase: "9", schedule: "Daily 7am" },
  inventory: { icon: Package, color: "amber", label: "Inventory Agent", phase: "9", schedule: "Daily 6am" },
  onboarding: { icon: UserPlus, color: "teal", label: "Onboarding Agent", phase: "9", schedule: "Event-driven" },
  enrichment: { icon: Globe, color: "indigo", label: "Enrichment Agent", phase: "9", schedule: "Daily 4am" },
  ...
  market_research: { icon: Globe, color: "indigo", label: "Market Research", phase: "4E", schedule: "Weekly Monday" },
...
export default function AgentDashboard({ companyId, onSelectAgent }) {
  ...
  const agentNames = status.registered_agents?.length
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Idjwi closed-loop operators</p>
          </div>
        </div>
      ...
      {!agentsEnabled && !statusLoading && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-800 flex items-center gap-2">
          ...
          <span>
            Agents are unavailable because the local Python layer or database is not ready.
            Optional LLM adviser keys are not required for Idjwi closed-loop agents.
          </span>
        </div>
      )}
      ...
      <p className="text-[10px] text-slate-400 text-center">
        Idjwi closed-loop agents run by default. External LLMs are optional advisers for deeper strategic analysis.
      </p>
    </div>
  );
}
