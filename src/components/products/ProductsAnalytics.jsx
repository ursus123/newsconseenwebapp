...
export default function ProductsAnalytics({ products = [], currentUser, standalone = false }) {
    ...
    <div className="mt-8 border border-slate-200 rounded-2xl overflow-hidden">
      ...
      {open && (
        <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
          <CategoryHeader title="Catalogue" icon="📦" />
          <ChartCard title="By Item Type" sql={`SELECT item_type, COUNT(*) FROM products GROUP BY item_type ORDER BY 2 DESC;`} currentUser={currentUser} entity="Products" tableData={d.byType}><ResponsiveContainer width="100%" height={180}><BarChart data={d.byType} margin={{left:-20}}><CartesianGrid strokeDasharray="3 3" opacity={0.3}/><XAxis dataKey="name" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar cursor="pointer" dataKey="count" radius={[4,4,0,0]}>{d.byType.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Bar></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="By Status" sql={`SELECT status, COUNT(*) FROM products GROUP BY status;`} currentUser={currentUser} entity="Products" tableData={d.byStatus}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byStatus} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="count" nameKey="name" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>{d.byStatus.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="By Unit of Measure" sql={`SELECT unit_of_measure, COUNT(*) FROM products GROUP BY unit_of_measure ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Products" tableData={d.byUOM}><ResponsiveContainer width="100%" height={180}><BarChart data={d.byUOM} layout="vertical" margin={{left:0,right:10}}><CartesianGrid strokeDasharray="3 3" opacity={0.3}/><XAxis type="number" tick={{fontSize:10}} allowDecimals={false}/><YAxis dataKey="name" type="category" tick={{fontSize:10}} width={70}/><Tooltip/><Bar cursor="pointer" dataKey="count" fill="#8b5cf6" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Item Class Distribution" sql={`SELECT item_class, COUNT(*) FROM products GROUP BY item_class ORDER BY 2 DESC;`} currentUser={currentUser} entity="Products" tableData={d.byClass}>{d.byClass.length>0?<ResponsiveContainer width="100%" height={180}><BarChart data={d.byClass} layout="vertical" margin={{left:0,right:10}}><CartesianGrid strokeDasharray="3 3" opacity={0.3}/><XAxis type="number" tick={{fontSize:10}} allowDecimals={false}/><YAxis dataKey="name" type="category" tick={{fontSize:10}} width={90}/><Tooltip/><Bar cursor="pointer" dataKey="count" fill="#06b6d4" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer>:<p className="text-xs text-slate-400 text-center py-16">No class data</p>}</ChartCard>
          <ChartCard title="By Brand (Top 10)" sql={`SELECT item_brand, COUNT(*) FROM products GROUP BY item_brand ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Products" tableData={d.byBrand}>{d.byBrand.length>0?<ResponsiveContainer width="100%" height={180}><BarChart data={d.byBrand} layout="vertical" margin={{left:0,right:10}}><CartesianGrid strokeDasharray="3 3" opacity={0.3}/><XAxis type="number" tick={{fontSize:10}} allowDecimals={false}/><YAxis dataKey="name" type="category" tick={{fontSize:10}} width={80}/><Tooltip/><Bar cursor="pointer" dataKey="count" fill="#f59e0b" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer>:<p className="text-xs text-slate-400 text-center py-16">No brand data</p>}</ChartCard>
          <ChartCard title="Item Subtype" sql={`SELECT item_subtype, COUNT(*) FROM products GROUP BY item_subtype ORDER BY 2 DESC LIMIT 10;`} currentUser={currentUser} entity="Products" tableData={d.bySubtype}>{d.bySubtype.length>0?<ResponsiveContainer width="100%" height={180}><BarChart data={d.bySubtype} layout="vertical" margin={{left:0,right:10}}><CartesianGrid strokeDasharray="3 3" opacity={0.3}/><XAxis type="number" tick={{fontSize:10}} allowDecimals={false}/><YAxis dataKey="name" type="category" tick={{fontSize:10}} width={100}/><Tooltip/><Bar cursor="pointer" dataKey="count" fill="#6366f1" radius={[0,4,4,0]}/></BarChart></ResponsiveContainer>:<p className="text-xs text-slate-400 text-center py-16">No subtype data</p>}</ChartCard>

          <CategoryHeader title="Inventory Health" icon="🏥" />
          <ChartCard title="Stock Level Distribution" sql={`SELECT CASE WHEN stock_quantity=0 THEN '0 (Out)' WHEN stock_quantity<=10 THEN '1-10' WHEN stock_quantity<=50 THEN '11-50' WHEN stock_quantity<=200 THEN '51-200' ELSE '200+' END bucket, COUNT(*) FROM products GROUP BY bucket;`} currentUser={currentUser} entity="Products" tableData={d.stockBuckets}><ResponsiveContainer width="100%" height={180}><BarChart data={d.stockBuckets} margin={{left:-20}}><XAxis dataKey="range" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar cursor="pointer" dataKey="count" radius={[4,4,0,0]}>{d.stockBuckets.map((_,i)=><Cell key={i} fill={["#ef4444","#f97316","#f59e0b","#10b981","#3b82f6"][i]}/>)}</Bar></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Expiry Status" sql={`SELECT CASE WHEN expiry_date<NOW() THEN 'Expired' WHEN expiry_date<NOW()+INTERVAL 30 DAY THEN '<30d' WHEN expiry_date<NOW()+INTERVAL 90 DAY THEN '<90d' ELSE 'OK' END s, COUNT(*) FROM products GROUP BY s;`} currentUser={currentUser} entity="Products" tableData={d.expiryBuckets}><ResponsiveContainer width="100%" height={180}><BarChart data={d.expiryBuckets} margin={{left:-20}}><CartesianGrid strokeDasharray="3 3" opacity={0.3}/><XAxis dataKey="status" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Bar cursor="pointer" dataKey="count" radius={[4,4,0,0]}>{d.expiryBuckets.map((_,i)=><Cell key={i} fill={["#ef4444","#f97316","#f59e0b","#10b981","#94a3b8"][i]||PALETTE[i%10]}/>)}</Bar></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Low Stock Items" description="Stock at or below reorder level" sql={`SELECT name FROM products WHERE stock_quantity IS NOT NULL AND reorder_level IS NOT NULL AND stock_quantity<=reorder_level ORDER BY stock_quantity ASC LIMIT 10;`} currentUser={currentUser} entity="Products" tableData={[{name:"Low Stock",count:d.lowStock},{name:"OK",count:products.length-d.lowStock}]}><div className="flex items-center justify-center min-h-[180px]"><div className="text-center"><p className="text-4xl font-bold text-rose-600">{d.lowStock}</p><p className="text-sm text-slate-500 mt-1">items at or below reorder level</p></div></div></ChartCard>
          <ChartCard title="With Expiry vs Without" sql={`SELECT CASE WHEN expiry_date IS NOT NULL THEN 'Has Expiry' ELSE 'No Expiry' END, COUNT(*) FROM products GROUP BY 1;`} currentUser={currentUser} entity="Products" tableData={[{name:"Has Expiry",count:d.withExpiry},{name:"No Expiry",count:d.noExpiry}]}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={[{name:"Has Expiry",count:d.withExpiry},{name:"No Expiry",count:d.noExpiry}]} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="count" nameKey="name" label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`} labelLine={false}>{[0,1].map(i=><Cell key={i} fill={PALETTE[i]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Stock Buckets Pie" sql={`SELECT CASE WHEN stock_quantity=0 THEN 'Out' WHEN stock_quantity<=10 THEN 'Critical' WHEN stock_quantity<=50 THEN 'Low' ELSE 'Adequate' END bucket, COUNT(*) FROM products GROUP BY bucket;`} currentUser={currentUser} entity="Products" tableData={d.stockBuckets}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.stockBuckets} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="range">{d.stockBuckets.map((_,i)=><Cell key={i} fill={["#ef4444","#f97316","#f59e0b","#10b981","#3b82f6"][i]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>

          <CategoryHeader title="Growth & Trends" icon="📈" />
          <ChartCard title="New Products by Month" sql={`SELECT DATE_TRUNC('month',created_date) m, COUNT(*) FROM products GROUP BY 1 ORDER BY 1;`} currentUser={currentUser} entity="Products" tableData={d.newByMonth}><ResponsiveContainer width="100%" height={180}><LineChart data={d.newByMonth} margin={{left:-20}}><CartesianGrid strokeDasharray="3 3" opacity={0.3}/><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Line type="monotone" dataKey="count" stroke="#10b981" strokeWidth={2} dot={false}/></LineChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Cumulative Product Count" sql={`SELECT DATE_TRUNC('month',created_date) AS month, COUNT(*) AS count FROM products WHERE created_date IS NOT NULL GROUP BY 1 ORDER BY 1;`} currentUser={currentUser} entity="Products" tableData={d.cumul}><ResponsiveContainer width="100%" height={180}><AreaChart data={d.cumul} margin={{left:-20}}><CartesianGrid strokeDasharray="3 3" opacity={0.3}/><XAxis dataKey="month" tick={{fontSize:10}}/><YAxis tick={{fontSize:10}} allowDecimals={false}/><Tooltip/><Area type="monotone" dataKey="total" stroke="#10b981" fill="#d1fae5"/></AreaChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Item Type Pie" sql={`SELECT item_type, COUNT(*) FROM products GROUP BY item_type;`} currentUser={currentUser} entity="Products" tableData={d.byType}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byType} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byType.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Status Pie" sql={`SELECT status, COUNT(*) FROM products GROUP BY status;`} currentUser={currentUser} entity="Products" tableData={d.byStatus}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byStatus} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byStatus.map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="UOM Pie" sql={`SELECT unit_of_measure, COUNT(*) FROM products GROUP BY unit_of_measure ORDER BY 2 DESC LIMIT 8;`} currentUser={currentUser} entity="Products" tableData={d.byUOM.slice(0,8)}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byUOM.slice(0,8)} cx="50%" cy="50%" innerRadius={30} outerRadius={70} dataKey="count" nameKey="name">{d.byUOM.slice(0,8).map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Class Pie" sql={`SELECT item_class, COUNT(*) FROM products GROUP BY item_class ORDER BY 2 DESC;`} currentUser={currentUser} entity="Products" tableData={d.byClass.slice(0,8)}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byClass.slice(0,8)} cx="50%" cy="50%" innerRadius={30} outerRadius={70} dataKey="count" nameKey="name">{d.byClass.slice(0,8).map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Brand Distribution" sql={`SELECT item_brand, COUNT(*) FROM products GROUP BY item_brand ORDER BY 2 DESC LIMIT 8;`} currentUser={currentUser} entity="Products" tableData={d.byBrand.slice(0,8)}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.byBrand.slice(0,8)} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="name">{d.byBrand.slice(0,8).map((_,i)=><Cell key={i} fill={PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Expiry Pie" sql={`SELECT CASE WHEN expiry_date<NOW() THEN 'Expired' WHEN expiry_date<NOW()+INTERVAL 30 DAY THEN '<30d' ELSE 'OK' END s, COUNT(*) FROM products GROUP BY s;`} currentUser={currentUser} entity="Products" tableData={d.expiryBuckets}><ResponsiveContainer width="100%" height={180}><PieChart><Pie data={d.expiryBuckets} cx="50%" cy="50%" outerRadius={75} dataKey="count" nameKey="status">{d.expiryBuckets.map((_,i)=><Cell key={i} fill={["#ef4444","#f97316","#f59e0b","#10b981","#94a3b8"][i]||PALETTE[i%10]}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></ChartCard>
        </div>
      )}
    </div>
  );
