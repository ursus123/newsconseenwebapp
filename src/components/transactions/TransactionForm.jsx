...
import { Save, X, Plus, Trash2, Upload, FileText, Package, Users, CreditCard, StickyNote } from "lucide-react";
...
const TABS = [
  ...
  { id: "payment", label: "Payment & Financials", icon: CreditCard },
...
export default function TransactionForm({ open, onClose, onSubmit, initialData }) {
  ...
  const updateLine = (idx, field, val) => set("line_items", (form.line_items || []).map((l, i) => i === idx ? { ...l, [field]: val } : l));
  ...
  const handlePost = () => {
    onSubmit({ ...form, amount: total, status: "posted" });
  };
  ...
  const renderTab = () => {
    switch (activeTab) {
        ...
        return (
          <div className="space-y-4">
            <Field label="Enterprise">
              <Input value={form.enterprise || ""} onChange={(e) => set("enterprise", e.target.value)} className="rounded-xl" placeholder="Enterprise name..." />
            </Field>
            <Field label="Reference Number">
              <Input value={form.reference_number || ""} onChange={(e) => set("reference_number", e.target.value)} className="rounded-xl" placeholder="Optional external ref" />
            </Field>
            ...
            <div className="space-y-2">
              ...
              {(form.line_items || []).map((line, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-center bg-slate-50 rounded-xl p-2">
                  <Input placeholder="Item name" value={line.item_name || ""} onChange={(e) => updateLine(i, "item_name", e.target.value)} className="col-span-4 rounded-lg text-xs h-8" />
                  <Input type="number" value={line.quantity ?? ""} onChange={(e) => updateLine(i, "quantity", parseFloat(e.target.value) || 0)} className="col-span-2 rounded-lg text-xs h-8" />
                  <Input placeholder="unit" value={line.unit || ""} onChange={(e) => updateLine(i, "unit", e.target.value)} className="col-span-2 rounded-lg text-xs h-8" />
                  <Input type="number" value={line.unit_price ?? ""} onChange={(e) => updateLine(i, "unit_price", parseFloat(e.target.value) || 0)} className="col-span-3 rounded-lg text-xs h-8" placeholder="0.00" />
      ...
      case "parties":
        return (
          <div className="space-y-4">
            <Field label="From Enterprise (optional)">
              <Input value={form.from_enterprise || ""} onChange={(e) => set("from_enterprise", e.target.value)} className="rounded-xl" placeholder="e.g. Warehouse" />
            </Field>
            <Field label="To Enterprise (optional)">
              <Input value={form.to_enterprise || ""} onChange={(e) => set("to_enterprise", e.target.value)} className="rounded-xl" placeholder="e.g. ABC Pharmacy" />
            </Field>
            <Field label="Assigned Person (optional)">
              <Input value={form.assigned_person || ""} onChange={(e) => set("assigned_person", e.target.value)} className="rounded-xl" placeholder="e.g. John Doe" />
            </Field>
            <Field label="Supplier / Customer (optional)">
              <Input value={form.supplier_customer || ""} onChange={(e) => set("supplier_customer", e.target.value)} className="rounded-xl" placeholder="e.g. Vendor X" />
            </Field>
            <div className="bg-blue-50 rounded-xl p-3 text-xs text-blue-600">
              <p className="font-semibold mb-1">Usage by type:</p>
              <p>• Stock Transfer → From + To enterprise</p>
              <p>• Sale / Service → Supplier / Customer</p>
              <p>• Item Assignment → Assigned Person</p>
            </div>
          </div>
        );
      ...
      case "payment":
        return (
          <div className="space-y-4">
            {!isFinancial && (
              <div className="bg-slate-50 rounded-xl p-3 text-xs text-slate-500 text-center">Payment details apply to financial transactions (Sale, Expense, Stock In/Out)</div>
            )}
            ...
            <Field label="Amount Paid">
              <Input type="number" value={form.amount_paid ?? total} onChange={(e) => set("amount_paid", parseFloat(e.target.value) || 0)} className="rounded-xl" placeholder="0.00" />
            </Field>
        ...
        <DialogHeader className="px-6 pt-5 pb-4 border-b border-slate-100">
          ...
          <div className="grid grid-cols-3 gap-3 mt-3">
            ...
            <div>
              ...
              <Input type="date" value={form.date || ""} onChange={(e) => set("date", e.target.value)} className="rounded-xl h-9 text-sm" />
            </div>
            <div>
              <Label className="text-xs text-slate-500 mb-1 block">Time</Label>
              <Input type="time" value={form.time || ""} onChange={(e) => set("time", e.target.value)} className="rounded-xl h-9 text-sm" />
            </div>
          </div>
        </DialogHeader>
