...
export default function UploadPanel({ uploadedTables, onTablesChange }) {
  ...
  const [previewData, setPreviewData] = useState(null); // { tableName, fileData }
  ...
  const readFile = async (file) => {
    ...
    try {
      ...
      if (file.name.match(/\.(xlsx|xls)$/i)) {
        // Upload and use AI extraction — get all sheets by extracting with a flexible schema
        const { file_url } = await base44.integrations.Core.UploadFile({ file });

        // Use InvokeLLM to extract sheets and their data
