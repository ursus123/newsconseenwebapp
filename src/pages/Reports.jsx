    ...
    return (
      <div>
        <h3 className={`text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-2 ${color}`}>
          <Icon className="w-3.5 h-3.5" /> {title} <span className="font-normal normal-case opacity-60">({items.length})</span>
        </h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map(chart => (
            <div key={chart.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col">
              {/* Card header */}
              <div className="px-4 pt-4 pb-2 flex items-start justify-between gap-2">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-800 truncate">{chart.title || "Untitled Chart"}</p>
                  {chart.description && <p className="text-[11px] text-slate-400 truncate mt-0.5">{chart.description}</p>}
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide ${
                    chart.source === "copilot" || chart.tool_name
              ...
              <div className="mt-auto px-4 pb-4 flex items-center justify-between">
                <p className="text-[10px] text-slate-400">
                  {chart.created_date ? new Date(chart.created_date).toLocaleDateString() : ""}
                </p>
                <button
                  onClick={() => onOpen(chart)}
                  className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition-colors"
                >
...
export default function Reports() {
  ...
  const [view, setView] = useState("folders"); // folders | chart-builder | report-builder | report-viewer | chart-viewer | ml-insights | market-template | superset | pinned-charts
  ...
  useEffect(() => {
    ...
    if (creates.length > 0) {
      Promise.allSettled(creates).then(() => qc.invalidateQueries({ queryKey: ["chartFolders"] }));
    }
  ...
  return (
    <div className="flex h-full bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        {/* Tab strip — visible on folders and pinned-charts views */}
        {(view === "folders" || view === "pinned-charts") && (
          <div className="flex items-center gap-2 px-4 pt-3 pb-0 border-b border-slate-100">
            ...
            <button
              onClick={() => setView("pinned-charts")}
              className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-t-lg transition-colors ${
                view === "pinned-charts"
        ...
        {showSetup ? (
          ...
          <PinnedChartsGrid
            charts={charts}
            currentUser={currentUser}
            onOpen={(chart) => { setEditingChart(chart); setView("chart-viewer"); }}
          />
        ) : view === "chart-viewer" ? (
          <ChartViewer
            chart={editingChart}
