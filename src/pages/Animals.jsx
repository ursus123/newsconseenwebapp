...
function StatCard({ icon: Icon, cls, label, value }) {
  return (
    <div className="relative bg-white border border-slate-100 rounded-2xl px-4 py-3 flex items-center gap-3">
      ...
      <div><p className="text-xs text-slate-400">{label}</p><p className="text-lg font-bold text-slate-800 leading-tight">{value}</p><AutonomousFigureExplainer entity="Animals" label={label} value={value} /></div>
    </div>
  );
}
...
export default function Animals() {
  ...
  const createMut = useMutation({
    mutationFn: (d) => dataService.createRecord("animal", d, currentUser, { queryClient: qc, notifyTaxonomyChange }),
    onSuccess: () => {
      setFormOpen(false); setEditing(null);
    },
  });
  const updateMut = useMutation({
    mutationFn: ({ id, data }) => dataService.updateRecord("animal", id, data, currentUser, { queryClient: qc, notifyTaxonomyChange, record: editing }),
    onSuccess: () => {
      setFormOpen(false); setEditing(null);
    },
  });
  const deleteMut = useMutation({
    mutationFn: (id) => dataService.deleteRecord("animal", id, currentUser, { queryClient: qc, record: deleting }),
    onSuccess: () => {
      setDeleting(null);
    },
  });
