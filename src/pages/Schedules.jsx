  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">{schedules.length} total schedules</p>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map(s => (
                <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-500 capitalize">{s.schedule_type || "—"}</td>
                  <td className="px-4 py-3 text-slate-500 capitalize">{s.frequency || "—"}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[s.status] || "bg-slate-100 text-slate-600"}`}>
                      {s.status || "—"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-slate-500">{s.start_date || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
