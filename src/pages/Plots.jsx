...
function StatCard({ icon: Icon, cls, label, value }) {
  return (
    <div className="relative bg-white border border-slate-100 rounded-2xl px-4 py-3 flex items-center gap-3">
      ...
      <div><p className="text-xs text-slate-400">{label}</p><p className="text-lg font-bold text-slate-800 leading-tight">{value}</p><AutonomousFigureExplainer entity="Plots" label={label} value={value} /></div>
    </div>
  );
}
