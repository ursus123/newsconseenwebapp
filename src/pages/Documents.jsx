...
const triggerETL = () =>
  fetch(`${RAILWAY_URL}/load/document-summary`, { method: "POST", headers: { "x-api-key": RAILWAY_API_KEY } }).catch(() => {});
...
function logAudit(companyId, action, record, userEmail) {
  fetch(`${RAILWAY_URL}/audit/log`, {
    ...
    body: JSON.stringify({ company_id: companyId, entity_type: "document", entity_id: record?.id, entity_name: record?.title || record?.id, action, changed_by: userEmail }),
  }).catch(() => {});
}
...
export default function Documents() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: (created) => {
      addRecordToQueryCache(qc, ["documents"], created);
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }) => base44.entities.Document.update(id, withScope(data)),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["documents"] });
      qc.refetchQueries({ queryKey: ["documents"] });
      triggerETL();
